 // ===========================================================
// TAYLER'S TESTING VERSION WITH MR. SYKORA
// ===========================================================
...
// Store an array: [0] = lower bound, [1] = upper bound
const double Hue[] = {73, 96};
const double Saturation[] = {145, 255};
const double Luminance[] = {63, 255};
//v4l2-ctl -d /dev/video0 -c exposure_auto=1 -c exposure_absolute=20
const double OBJECT_WIDTH = 2; //Area of the tracking tape in real life
const double PIXEL_WIDTH = 40; //Area of the pixels at base distance
...
// Finds targets
void findTargets(Mat &imageInput, vector<vector<Point> > &input, Mat &output, double &avgDistance, double &angleCorrelation, double &offset){	
	...
	if (boundingBoxes.size() >= 2) {
		...
		Rect left = Rect(0,0,outputSize.width/2,outputSize.height);
		Rect right = Rect(outputSize.width/2,0,outputSize.width/2,outputSize.height);
		Rect leftTarget = ((left & target1).area() > 0) ? target1 : target2;
		Rect rightTarget = ((right & target1).area() > 0) ? target1 : target2;
		
		//Draws two rectanges onto output
		rectangle(output, leftTarget, GREEN, 2);
		rectangle(output, rightTarget, YELLOW, 2);
		...
		double leftWidth = leftTarget.width;
		double rightWidth =	rightTarget.width;
		double leftDistance = (OBJECT_WIDTH * FOCAL_LENGTH)/leftWidth;
		double rightDistance = (OBJECT_WIDTH * FOCAL_LENGTH)/rightWidth;
		double areaRatio = .99*(leftWidth/rightWidth); //some constant ie .99
		angleCorrelation = 1-areaRatio;
		...
		putText(output, "Offset: "+ to_string(offset), Point2f(30,2*15), cv::FONT_HERSHEY_PLAIN, 0.8, WHITE, 1);
		...
		putText(output, "Left Target Distance: "+ to_string(leftDistance), Point2f(30,6*15), FONT_HERSHEY_PLAIN, 0.8, WHITE, 1);
		putText(output, "Right Target Distance: "+ to_string(rightDistance), Point2f(30,7*15), FONT_HERSHEY_PLAIN, 0.8, WHITE, 1);
		putText(output, "Left Target Width: "+ to_string(leftWidth), Point2f(30, 8*15), FONT_HERSHEY_PLAIN, 0.8, WHITE, 1);
		putText(output, "Right Target Width: "+ to_string(rightWidth), Point2f(30, 9*15), FONT_HERSHEY_PLAIN, 0.8, WHITE, 1);
		putText(output, "Area Ratio: "+ to_string(areaRatio), Point2f(30, 10*15), FONT_HERSHEY_PLAIN, 0.8, WHITE, 1);
		putText(output, "Angle Correlation: "+ to_string(angleCorrelation), Point2f(30, 11*15), FONT_HERSHEY_PLAIN, 0.8, WHITE, 1);
	}
}

// Does the image processing
void processImage(Mat& input, Mat& output, double &distance, double &angleCorrelation, double &offset){
	...
	findTargets(maskOutput, contoursValueOutput, targetsOutput, distance, angleCorrelation, offset);
...
void PublishNetworkTables(shared_ptr<NetworkTable> table, double distance, double angleCorrelation, double offset) {
	table->PutNumber("Distance", distance);
	table->PutNumber("Offset", offset);
	table->PutNumber("Angle Correlation", angleCorrelation);
}
...
int main(int argc, char *argv[]) {
	...
	while (!quit()) {
		...
		processImage(raw, processed, distance, angleCorrelation, offset);
		// Publisheds Data to NetworkTable - Vision
		PublishNetworkTables(ntable, distance, angleCorrelation, offset);
		// Runs if debug is true
		if(debug){
			// Display processed image
			imshow("Processed image", processed);
			// Output data to console
			cout << "Distance: "<< distance << "\tAngle Correlation: " << angleCorrelation << endl << "\tOffset: " << offset << endl;
		}
	}
	NetworkTable::Shutdown();
